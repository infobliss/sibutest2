...
class NationaalArchief(GenericGLAM):
    ...
    def choose_correct_template(url):
        val, dict = load_json_from_url(url)
        if val==0:
            return 0
        else:
            #ToDo: do some processing to choose the right template, return 1 if unable to decide
            return 1

    def fill_template(url, category):
        categories=[category]
        right_template = choose_correct_template(url)
        if right_template == 0:
            print("Incorrect URL given")
        else:
            #perform the glam specific metadata mapping here and form the dictionary
            dict['photographer'] = parsed_j["doc"]["Vervaardiger"][0]
            ...
            if not title:
                ...
                dict['title'] = title
            #ToDo: Add the language tag by guessing the language of the title and description
            ...
            dict['accession_number'] = archiefinventaris + ' (archive inventory number), ' + identifier + ' (file number)'
            dict = {'artist':'Myartist', 'title': 'Mytitle', 'description': 'Mydescription', 'photographer': 'Myphotographer'}
            collectionname=department
            UUID=parsed_j["doc"]["id"]
            dict['source'] = 'Nationaal Archief, ' + collectionname + ', {{Nationaal Archief-source|UUID=' + UUID + '|file_share_id='+ identifier
        ...
        if categories[0]:
            category_text = '[[Category:Images from the Nationaal Archief needing categories]]\n'

        elif hasPhotographerInDict:
            category_text = '[[Category:Photographs by ' + photographerName + ']]\n'
